...
(function($) {
		...
		colPosition = 0; // for printing colors on simple bar graph
 		...
 		for(var val in data){
			...
			unique = val+el.id; //unique identifier
 			...
 			out  = "<div lang=exp class='graphField"+el.id+"' id='graphField"+unique+"' style='position: absolute'>";
 			out += "<div lang=exp class='graphValue"+el.id+"' id='graphValue"+unique+"'>"+prefix+value+postfix+"</div>";
 			
 			out += "<div lang=exp class='graphBar"+el.id+"' id='graphFieldBar"+unique+"' style='background-color:"+color+";position: relative; overflow: hidden;'></div>";

			// if there is no legend or exist legends display lbl at the bottom
 			if(!arr.legend || arr.legends)
 				out += "<div lang=exp class='graphLabel"+el.id+"' id='graphLabel"+unique+"'>"+lbl+"</div>";
 			out += "</div>";
 			...
 			if(valueData instanceof Array){
				...
				if(arr.type=="multi"){
 				...
 				for (i in valueData){
 					...
 					o = "<div lang=exp class='subBars"+el.id+"' style='height:"+heig+"px; background-color: "+arr.colors[i]+"; left:"+wid*i+"px; color:"+arr.showValuesColor+"; font-size:"+fs+"px' >"+sv+"</div>";
 		...
 		//position of legend
 		if(arr.legend){
			$(el).append("<div lang=exp id='legendHolder"+unique+"'></div>");
 		...
 		//position of title
 		if(arr.title){
 			$(el).wrap("<div lang=exp id='graphHolder"+unique+"'></div>");
	...
	createLegend = function(legendArr){
		...
		for(var val in legendArr){
	 			legend += "<div lang=exp id='legend"+legendArr[val][3]+"' style='overflow: hidden; zoom: 1;'>";
	 			legend += "<div lang=exp class='legendBar"+legendArr[val][2]+"' id='legendColor"+legendArr[val][3]+"' style='background-color:"+legendArr[val][0]+"'></div>";
	 			legend += "<div lang=exp class='legendLabel"+legendArr[val][2]+"' id='graphLabel"+unique+"'>"+legendArr[val][1]+"</div>";
	 			legend += "</div>";			
		}
	};
